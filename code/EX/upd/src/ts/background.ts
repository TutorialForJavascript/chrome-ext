...
function receiveMsg(info:ChromeAppSocket.UDPReceiveEventArgs){
...
let udpSocket = new ChromeAppSocket.UPD(undefined,54322,undefined,receiveMsg)
chrome.runtime.onMessage.addListener((message, sender, callback)=>{
    if(message.action == 'send'){
        ...
        console.log(message.msg)
        console.log(buf)
        udpSocket.send( buf,'0.0.0.0', 54321, function(info:ChromeAppSocket.SendInfo){
            console.log(info)
        })
    }
})
